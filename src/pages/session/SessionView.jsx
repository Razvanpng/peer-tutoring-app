...
function MessageBubble({ message, isOwn }) {
  return (
    <div className={`flex items-end gap-2 ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      >
        <p>{message.content}</p>
        <p className={`text-[10px] mt-1 ${isOwn ? 'text-slate-400' : 'text-slate-400'} text-right`}>
...
function SessionHeader({ session, onBack }) {
  const STATUS_STYLES = {
  ...
  return (
    <header className="bg-white border-b border-slate-200 shrink-0">
      <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
        ...
        <span className={`shrink-0 text-xs font-medium border rounded-full px-2.5 py-0.5 capitalize ${style}`}>
          {session.status}
        </span>
      </div>
    </header>
...
export default function SessionView() {
  ...
  return (
    <div className="h-screen bg-slate-50 flex flex-col">
      ...
      <SessionHeader session={session} onBack={() => navigate(-1)} />
